...
Usage:
  python3 tools/mock_receiver.py [--host 0.0.0.0] [--port 9000]
                                  ...
                                  [--lifecycle sleeping|closing]
                                  [--lifecycle-after N]
                                  [--hello-delay SECONDS] [--no-hello-secs SECONDS]
                                  [--protocol-version N]
...
def send_liveness_pings(sock: socket.socket, stop: threading.Event,
                        sent: dict[int, float]) -> None:
    """Mirrors the receiver's normative ~2 second ping cadence."""
    try:
        while not stop.wait(2.0):
            timestamp = int(time.time() * 1000)
            sent[timestamp] = time.monotonic()
            send_control(sock, type="ping", t=timestamp)
    except OSError:
        return
...
def main() -> int:
    ...
    ap.add_argument("--lifecycle", choices=("sleeping", "closing"), default=None,
                    help="send a protocol lifecycle message and close")
    ap.add_argument("--lifecycle-after", type=int, default=30,
                    help="video frame after which --lifecycle is sent")
    ap.add_argument("--hello-delay", type=float, default=0.0,
                    help="wait before sending hello, to exercise the visible handshake state")
    ap.add_argument("--no-hello-secs", type=float, default=0.0,
                    help="accept a connection but send no hello, then close after N seconds")
    ap.add_argument("--protocol-version", type=int, default=3,
                    help="protocol version advertised in hello")
    args = ap.parse_args()
    ...
    if args.no_hello_secs > 0:
        print(f"[mock] intentionally withholding hello for {args.no_hello_secs:.1f}s")
        time.sleep(args.no_hello_secs)
        conn.close()
        srv.close()
        print("[mock] closed no-hello scenario")
        return 0

    if args.hello_delay > 0:
        print(f"[mock] delaying hello for {args.hello_delay:.1f}s")
    ...
    def hello_json(w: int, h: int) -> str:
        return ('{"type":"hello","pixelsWide":%d,"pixelsHigh":%d,"scale":%d,'
                '"device":"iPad","id":"mock-uuid-0000","pv":%d}' %
                (w, h, args.scale, args.protocol_version))
    ...
    sent_pings: dict[int, float] = {}
    threading.Thread(target=send_liveness_pings,
                     args=(conn, ping_stop, sent_pings), daemon=True).start()
        ...
        while True:
            ...
            if is_control(payload):
                ...
                try:
                    ...
                    raise AssertionError(f"invalid control JSON from sender: {exc}") from exc
                if control.get("type") == "welcome":
                    welcome_seen.set()
                elif control.get("type") == "pong":
                    echoed = control.get("t")
                    if echoed not in sent_pings:
                        raise AssertionError(f"pong echoed an unknown t value: {echoed!r}")
                    if not isinstance(control.get("mt"), (int, float)):
                        raise AssertionError("pong is missing numeric mt")
                    rtt_ms = (time.monotonic() - sent_pings.pop(echoed)) * 1000
                    print(f"[mock] pong validated: t={echoed}, rtt={rtt_ms:.1f}ms")
                continue
            ...
            if args.lifecycle and video_frames >= args.lifecycle_after:
                send_control(conn, type=args.lifecycle)
                print(f"[mock] sent lifecycle message: {args.lifecycle}")
                time.sleep(0.1)
                break
