...
private:
    ...
    // Claims the active Parsec DisplayConfig target path that appeared since
    // `pathKeysBefore` was snapshotted, maps it to its still-inactive outer GDI
    // source name, then enables/places/sizes that source.
    bool FindMonitorGeometry(const std::set<std::wstring>& pathKeysBefore);
    void KeepAliveLoop();
    ...
    mutable std::mutex stateMutex_; // guards cached geometry and claimed display/path identity
    RECT monitorRect_{};
    std::wstring deviceName_;
    std::wstring monitorPathKey_;
};
