...
enum : uint16_t { kTypeA = 1, kTypePtr = 12, kTypeTxt = 16, kTypeAaaa = 28, kTypeSrv = 33 };
...
void ParseResponse(const uint8_t* data, size_t size, std::map<std::string, MdnsReceiver>& found)
{
    ...
    auto readUint32 = [&](size_t at) -> uint32_t {
        return (static_cast<uint32_t>(data[at]) << 24) | (static_cast<uint32_t>(data[at + 1]) << 16) |
    ...
    struct HostAddress { std::vector<std::string> addresses; uint32_t ttl = 120; };
    std::map<std::string, HostAddress> addressByHost;

    for (size_t i = 0; i < records; ++i) {
        if (!ReadName(data, size, pos, name) || pos + 10 > size)
            return;
        uint16_t type = readUint16(pos);
        uint32_t ttl = readUint32(pos + 4);
        ...
        switch (type) {
            case kTypePtr: {
                ...
                if (ReadName(data, size, cursor, instance)) {
                    MdnsReceiver& receiver = found[instance];
                    receiver.instance = instance.substr(0, instance.find('.'));
                    receiver.name = receiver.instance;
                    receiver.ttlSeconds = ttl;
                    receiver.goodbye = ttl == 0;
                }
                break;
            }
            case kTypeSrv: {
                ...
                receiver.ttlSeconds = (std::min)(receiver.ttlSeconds, ttl);
                receiver.goodbye = receiver.goodbye || ttl == 0;
                std::string host;
            ...
            case kTypeA: {
                ...
                HostAddress& hostAddress = addressByHost[name];
                hostAddress.addresses.push_back(text);
                hostAddress.ttl = (std::min)(hostAddress.ttl, ttl);
                break;
            }
            case kTypeAaaa: {
                if (dataLength != 16) break;
                char text[INET6_ADDRSTRLEN] = {};
                in6_addr addr{};
                std::memcpy(&addr, data + recordStart, 16);
    ...
    for (auto& entry : found) {
        ...
        if (address != addressByHost.end()) {
            entry.second.addresses = address->second.addresses;
            // Prefer IPv4 while link-local IPv6 advertisements do not carry a
            // Windows scope id in the DNS payload. Numeric IPv6 remains a
            // supported manual/hostname-resolved fallback.
            auto ipv4 = std::find_if(entry.second.addresses.begin(), entry.second.addresses.end(),
                                     [](const std::string& value) { return value.find(':') == std::string::npos; });
            entry.second.address = ipv4 != entry.second.addresses.end() ? *ipv4 : entry.second.addresses.front();
            entry.second.ttlSeconds = (std::min)(entry.second.ttlSeconds, address->second.ttl);
            entry.second.goodbye = entry.second.goodbye || address->second.ttl == 0;
        }
        entry.second.online = !entry.second.goodbye;
    }
}
...
void AppendRecord(std::vector<uint8_t>& out, const std::string& name, uint16_t type,
                  const std::vector<uint8_t>& payload, uint32_t ttl = 120)
{
    AppendName(out, name);
    PutUint16(out, type);
    PutUint16(out, 1);   // class IN
    PutUint16(out, static_cast<uint16_t>(ttl >> 16));
    PutUint16(out, static_cast<uint16_t>(ttl & 0xFFFF));
...
bool SelfCheck()
{
    ...
    if (receiver.instance != "iPad" || receiver.host != "iPad-Pro.local" || receiver.port != 9000 ||
        ...
        receiver.ttlSeconds != 120 || receiver.goodbye)
        return fail("a well-formed response was parsed into the wrong fields");

    std::vector<uint8_t> goodbye = ResponseHeader(1);
    std::vector<uint8_t> goodbyePtr;
    AppendName(goodbyePtr, "iPad._opensidecar._tcp.local");
    AppendRecord(goodbye, kService, kTypePtr, goodbyePtr, 0);
    found.clear();
    ParseResponse(goodbye.data(), goodbye.size(), found);
    if (found.empty() || !found.begin()->second.goodbye || found.begin()->second.online)
        return fail("TTL=0 goodbye did not mark the receiver offline");
    ...
    printf("mdns self-check ok: well-formed, TTL/goodbye, overlong TXT, %zu truncations, pointer loop, lying header\n",
           good.size() - 1);
    return true;
}

void DiscoveryCache::Merge(const std::vector<DiscoveryRecord>& records, std::chrono::steady_clock::time_point now)
{
    for (DiscoveryRecord incoming : records) {
        const auto matches = [&](const DiscoveryRecord& existing) {
            if (!incoming.id.empty() && existing.id == incoming.id) return true;
            return incoming.id.empty() && !incoming.instance.empty() && existing.instance == incoming.instance;
        };
        auto it = std::find_if(records_.begin(), records_.end(), matches);
        if (incoming.goodbye || incoming.ttlSeconds == 0) {
            if (it != records_.end()) {
                it->online = false;
                it->goodbye = true;
                it->lastSeen = now;
        ...
        incoming.goodbye = false;
        incoming.lastSeen = now;
        incoming.expiresAt = now + std::chrono::seconds(incoming.ttlSeconds == 0 ? 1 : incoming.ttlSeconds);
        if (incoming.addresses.empty() && !incoming.address.empty()) incoming.addresses.push_back(incoming.address);
        if (it == records_.end()) {
            records_.push_back(std::move(incoming));
        } else {
            // A duplicate stable id is one device at a new address, not a new
            // sender. Preserve older addresses only as ordered fallbacks.
            for (const std::string& address : it->addresses)
                if (std::find(incoming.addresses.begin(), incoming.addresses.end(), address) == incoming.addresses.end())
                    incoming.addresses.push_back(address);
            *it = std::move(incoming);
        }
    }
}

void DiscoveryCache::Expire(std::chrono::steady_clock::time_point now)
{
    for (DiscoveryRecord& record : records_)
        if (record.online && record.expiresAt != std::chrono::steady_clock::time_point{} && now >= record.expiresAt)
            record.online = false;
}

std::optional<DiscoveryRecord> DiscoveryCache::FindById(const std::string& id) const
{
    if (id.empty()) return std::nullopt;
    auto it = std::find_if(records_.begin(), records_.end(), [&](const DiscoveryRecord& record) { return record.id == id; });
    return it == records_.end() ? std::nullopt : std::optional<DiscoveryRecord>(*it);
}

std::vector<DiscoveryRecord> DiscoveryCache::Records() const
{
    return records_;
}
...
std::vector<MdnsReceiver> BrowseReceivers(int timeoutMs)
{
    ...
    for (auto& entry : found) {
        ...
        if (!entry.second.address.empty() || entry.second.goodbye) {
            entry.second.lastSeen = now;
            entry.second.expiresAt = now + std::chrono::seconds(entry.second.ttlSeconds);
