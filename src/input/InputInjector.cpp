...
{
    azimuth = std::clamp(azimuth, -2.0 * kPi, 2.0 * kPi);
    altitude = std::clamp(altitude, 0.0, kPi / 2.0);
...
void InputInjector::SetMonitorRect(const RECT& rect)
{
    std::lock_guard<std::mutex> lock(rectMutex_);
    monitorRect_ = rect;
}

RECT InputInjector::MonitorRect() const
{
    std::lock_guard<std::mutex> lock(rectMutex_);
    return monitorRect_;
}
...
void InputInjector::HandleTouch(const TouchMsg& touch)
{
    ...
    LONG screenX = rect.left + static_cast<LONG>(Normalized(touch.x) * (rect.right - rect.left));
    LONG screenY = rect.top + static_cast<LONG>(Normalized(touch.y) * (rect.bottom - rect.top));
...
{
    if (scroll.dy != 0.0) {
        double pixels = std::clamp(scroll.dy, -10'000.0, 10'000.0);
        LONG delta = static_cast<LONG>((pixels / kPixelsPerWheelNotch) * WHEEL_DELTA);
        if (delta != 0)
            SendMouseInput(MOUSEEVENTF_WHEEL, 0, 0, delta);
    }
    if (scroll.dx != 0.0) {
        double pixels = std::clamp(scroll.dx, -10'000.0, 10'000.0);
        LONG delta = static_cast<LONG>((pixels / kPixelsPerWheelNotch) * WHEEL_DELTA);
        if (delta != 0)
...
POINT InputInjector::ScreenPoint(double nx, double ny) const
{
    ...
    RECT rect = MonitorRect();
    POINT pt;
    pt.x = rect.left + std::lround(Normalized(nx) * (rect.right - rect.left)) -
           GetSystemMetrics(SM_XVIRTUALSCREEN);
    pt.y = rect.top + std::lround(Normalized(ny) * (rect.bottom - rect.top)) -
...
{
    // A link can disappear between touch began/ended. Always release the
    // synthetic mouse button, even on systems where no pen device was ever
    // created, otherwise Windows keeps dragging after the iPad disconnects.
    if (isDown_) {
